...
public:
	...
	void help(TreeNode* p, int depth, int index, vector<int>& v, int& max){
		if(!p){
			return;
		}
		...
		int width = index - v[depth] + 1;
		if(width > max){
			max = width;
		}

		help(p->left, depth + 1, index * 2 - 1, v, max);
		help(p->right, depth + 1, index * 2, v, max);
	}
};
