...
class Solution {
public:
	int helper(int id, const map<int, Employee*>& m){
		map<int, Employee*>::const_iterator it = m.find(id);
		if(it == m.end()){
			return 0;
		}

		const Employee* p = it->second;
		int importance = p->importance;
		for(auto _id : p->subordinates){
			importance += helper(_id, m);
		}
